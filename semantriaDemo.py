...
#Define custom class to stare id title pairs
...
#     def getText(self, idTextPair):
#         return idTextPair.text
    
# def IDtoTitle(ID, data, idList, titleList):
...
#initialize two lists to hold file contents
fileIDs = []
fileSynopses = []
...
#ID to Title mapping - this is ~80 000 entries
mat = spio.loadmat(r'movie_metadata.mat', struct_as_record=False)
data = mat['m']
idList = (data[0,0].ID).tolist()
#dListCopy = (data[0,0].ID).tolist()
for index, id in enumerate(idList):
    idList[index] = idList[index][0] #transform list of lists into single list of ids
    #idListCopy[index] = idListCopy[index][0]
shitTitleList = (data[0,0].Title).tolist()
titleList = []
for title in shitTitleList:
    goodTitle = title.encode('ascii','ignore')
    goodTitle = (goodTitle[:-1]).strip()
...
for entry in movieScores:
    print(entry+","+str(movieScores[entry][0])+","+movieScores[entry][1])

#write results to a file
f = open("database.txt", "w")
for entry in movieScores:
    f.write(entry+":"+str(movieScores[entry][0])+":"+movieScores[entry][1]+"\n")
